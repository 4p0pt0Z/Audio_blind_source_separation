...
def generate_segments(all_wavs_filenames, all_labels_filenames, config, classes,
    ...
    r"""Produces the audio files of the segments from the audio files from Audioset and their label files,
        and compute the associated features. Save the audio data and return the features.

    Args:
        all_wavs_filenames (list): List of all the wav files from Audioset
        all_labels_filenames (list): List of all the label files from CloudFactory for the files in
        all_wavs_filenames
    ...
    # Prepare queues for the segments information
    ...
    for idx, (audio_file, label_file) in enumerate(zip(all_wavs_filenames, all_labels_filenames)):
        ...
        for seg_idx in range(len(segments)):
            if config['Add_gaussian_noise']:  # Add gaussian white noise
            ...
            try:  # Compute duration of the labelled portion of the segment.
            ...
            # The segment get a label for a class if at least 10 frame in the segment has the label
            seg_labels = (np.array(durations[-1]) >= 10.0 * config["STFT_frame_width_ms"] / 1000.0)
...
def save_features(save_path, stft_magnitudes, stft_phases, mel_spectrograms, labels, segment_names, durations):
    r"""Saves the segments audio features and information to hdf5.

    Args:
        save_path (str): path to hdf5 file to save the data.
        stft_magnitudes (np.ndarray): Segments magnitudes
        stft_phases (np.ndarray): Segments phases
        mel_spectrograms (np.ndarray): Segments mel Spectrograms
        labels (np.ndarray): Segments labels
...
def main():
    r"""Generate audio segments and corresponding labels from Audioset files and CloudFactory labels.
        ...
        The script also computes the STFT and Mel spectrogram of each segments. Once the segments are split in
        training, testing and validation sets these information are saved to hdf5 files.

        WARNING: The data set labelled by Cloud factory is relatevely big and this scripts needs at least 25 GB or
        RAM to run !
    """

    config = {"length_segments_s": 3.3,  # Lengths in seconds of the segments
              ...
              # Parameters for the audio features extraction.
              ...
              # The script can also add gaussian white noise to each segment.
    ...
    # Round the user passed arguments so that the produced segments contain an integer number of fft frames.
    ...
    # Create output folder. If it already exists and is not empty: raise Error.
    ...
    # save config for the audio features extraction.
    with open(os.path.join(config["output_folder"], 'config.json'), 'w') as config_file:
        json.dump(config, config_file)

    # Generate the audio segments features (save the audio segments to files)
    stft_magnitudes, stft_phases, mel_spectrograms, labels, segment_names, durations = \
    ...
    # Remove the duplicates segments
    stft_magnitudes, stft_phases, mel_spectrograms, labels, segment_names, durations = \
    ...
    # Save all the segments features to an hdf5 file.
    ...
    # Make training, testing and validation set from all the segments, and save the features for each set to hdf5.
